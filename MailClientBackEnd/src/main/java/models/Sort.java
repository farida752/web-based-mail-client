...
public void SortTmpListForSearching(DLinkedList l) {
	...
	else if(sortAccordingTo.equalsIgnoreCase("dateDesc")||sortAccordingTo.equalsIgnoreCase("none")) {
		if(l.size()>0) {
		String arrOfDates[]=new String[l.size()];int counter =0;
		Dnode v=l.head.getNext();
		while(counter<l.size()) {
...
public void SortAccToPriority(DLinkedList list) {
	...
	while(counter<list.size()) {//"Urgent/Important", "Not Urgent/Important", "Urgent/Not Important", "Not Urgent/Not Important"
		if(d.getElement().getPriority().equalsIgnoreCase("Urgent/Important")) {
			key=1;
		}else if(d.getElement().getPriority().equalsIgnoreCase("Not Urgent/Important")) {
			key=2;
		}else if(d.getElement().getPriority().equalsIgnoreCase("Urgent/Not Important")) {
			key=3;
...
public String[] sortContact(LinkedList contacts) {

	
	String[]email= new String[contacts.size()];
	
	for(int i=0;i<contacts.size();i++) {
		email[i]=(String)contacts.get(i);			
	}
	
	 if(sortAccordingTo.equalsIgnoreCase("e-mail")) {
		email = Quicksort1(email);
	}
	 return email;
